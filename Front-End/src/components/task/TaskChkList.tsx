    ...
    <div>
      ...
      <div className="flex progress-bar-container" style={{ margin: "20px 0" }}>
        <div className="w-full bg-gray-200 rounded-full h-4 dark:bg-gray-700 mb-4">
          <div
            className="bg-main-color h-4 rounded-full"
        ...
        {checklist.map((item) => (
          <li key={item.checkListId} className="flex items-center mb-2">
            <input
              ...
              onChange={() => handleChecklistChange(item.checkListId)}
            />
            <span className={item.isFinished ? "line-through ml-2" : "ml-2"}>
              {item.title}
            </span>
            <button
              className="ml-auto text-red-500"
              onClick={() => removeChecklistItem(item.checkListId)}
            >
              X
            </button>
          </li>
