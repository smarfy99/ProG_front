        ...
        <div className="bg-gray-100">
                ...
                <div className="flex flex-col px-5 py-2" key={comment.id}>
                    <div className="px-2 py-2">
                        <div className="flex items-center">
                            <ImageWithFallback src={comment.member.imgUrl} alt='Profile' type='member' style='flex-none w-12 h-12 rounded-full'/>
                            ...
                            {!comment.isDeleted && comment.member.id === memberId && <div className="flex-initial w-32">
                                <button className="p-2 text-sm rounded"
                                        onClick={() => changeEditor(index, comment.content)}>수정
                                </button>
                                <button className="p-2 text-sm rounded" onClick={() => deleteComment(comment.id)}>삭제
                                </button>
                            </div>}
                        </div>
                    </div>
                                    ...
                                    <div className="my-3">
                                        <div className="flex flex-col">
                                            ...
                                            <div className="flex justify-end space-x-2">
                                                <button className="bg-main-color text-white p-2 w-20 rounded"
                                                        onClick={() => updateComment(comment.id, index)}>저장
                                                </button>
                                                <button className="bg-red-400 text-white p-2 w-20 rounded"
            ...
            <div className="px-5 py-2">
                <div className="flex flex-col p-2">
                    <div className="flex">
                        <ImageWithFallback src={profile?.imgUrl || ''} alt='Profile' type='member' style='flex-none w-12 h-12 rounded-full'/>
