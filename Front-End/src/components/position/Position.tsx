        ...
        const getPositionList = async () => {
            try {
                ...
                if (response.data.status === 'OK') {
                    setPositionList(
                        response.data.data.map(({id, detailDescription}: {
                            id: number;
            ...
            <div className='my-3'>
                ...
                <div>
                    {state.positionDetailDescription.map((_description, index) => (
                        <div key={index} className='flex items-center mb-2'>
                            <button
                                ...
                                className='mr-2 p-3 pr-3 text-white rounded-lg'>
                                ➖
                            </button>
                            <select
                            ...
                            >
                                ...
                                {positionList.map((option) => (
                                    <option key={option.id} value={option.id}
                                            disabled={selectedPositionIds.includes(option.id)}>
                    ...
                    <button onClick={handleAddPosition}
                            className='p-2 text-white rounded-lg position-add-btn'>
                        포지션 추가
                    </button>
                </div>
            </div>
