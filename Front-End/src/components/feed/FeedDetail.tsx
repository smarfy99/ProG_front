  ...
  const getFreeFeed = async () => {
    try {
      const response = await axiosInstance.get(`/boards/detail/${boardId}`, {
        headers: {
          Authorization:
            "Bearer eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJpZCI6MSwiZXhwIjozODUzOTA4NDgwLCJpYXQiOjE3MDY0MjQ4MzMsImF1dGhvcml0aWVzIjpbIlJPTEVfVVNFUiJdfQ.vXlMCRdnAL60yLcAtV70jgpKfYvKBlwSu-NFrCI9LSI",
        },
      });
          ...
          <div className="flex gap-2">
            ...
            {showModiDelete && (
              <div className="row absolute 40 right-0 top-auto bg-white shadow-md rounded-md mt-1 z-40">
                <button className="w-40 mt-2" onClick={handleModifyClick}>
                  수정하기
                </button>
                <button className="w-40 mt-2" onClick={handleDelete}>
                  삭제하기
                </button>
              </div>
            )}
            {showModiDetail && (
              <div className="z-40">
                <ModiDetail boardId={boardId} onClose={handleCloseModiDetail} />
              </div>
            )}{" "}
          </div>
        </div>
