    ...
    const getData = async () => {
        try {
            ...
            membersData.forEach((memberData: MemberData) => {
                ...
                const position = updatedPositions.find((pos: {
                    posCode: any
                }) => pos.posCode === memberData.jobCode.id);
                if (position) {
            ...
            <div className={'project-box'}>
                <div className={'flex justify-between under-line'}>
                    ...
                    <div className={'grid grid-cols-2 gap-x-5'}>
                        {isProjectStarted ?
                            (<button className={'project-home-btn end'} onClick={endProject}>프로젝트 종료</button>)
                            :
                            (<button className={'project-home-btn'} onClick={startProject}>프로젝트 시작</button>)
                        }
                        {/*<p>참여인원</p> /!*참여인원 => 컴포넌트 제작하기*!/*/}
                        <button className={'project-home-btn'} onClick={Setting}>설정</button>
                    </div>
                </div>
                <div className={'p-3'}>
                    <button className={'my-work-btn'} onClick={() => onHandleClick(navigate, projectId||'')}>🔎 내 업무 {myHomeInfo?.myWorkCnt}</button>
                    <div>
