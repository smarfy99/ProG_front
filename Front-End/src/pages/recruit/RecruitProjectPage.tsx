          ...
          <div className='h-auto border-2 border-black rounded-3xl p-5'>
            ...
            <div className='flex justify-between m-5'>
              ...
              <div className='flex-col w-3/12 h-52'>
                <div className='border-2 border-black h-24 mb-4 rounded-lg p-2'>예상기간
                  <div className='text-center text-3xl mt-3'>{period} 주</div>
                </div>
                <div className='border-2 border-black h-24 rounded-lg p-2'> 총 인원
                      ...
                      {positions.posName.map((name, index) => (
                        <div key={index} className='h-8 m-1'>
                          ...
                          {positions.posNowNumber[index] < positions.posNumber[index] ? (
                            <>
                              {getApply === '' ? (
                                <button className='ml-3 bg-main-color text-white p-1 rounded ' onClick={() => apply(name, positions.posCode[index])}>
                                  지원하기
                                </button>
                              ) : getApply === positions.posCode[index] ? (
                                <button className='ml-3 bg-red-300 text-white p-1 rounded' onClick={() => cancelApply()}>
                                  취소하기
                                </button>
                              ) : (
            ...
            <div className=' m-5 flex justify-center'>
              ...
              <div className={`border-black border-2 m-5 w-32 h-28 rounded-3xl p-2 cursor-pointer ${isLike === 1 ? 'bg-sub-color' : ''}`} onClick={toggleLike}>
                <div className='flex justify-center'>
            ...
            <hr className='m-3 border-main-color border-1' />
            <div className='border-2 border-black m-5'><Comment /></div>
